...
export default function ForgotPasswordScreen() {
  ...
  const handleSend = async () => {
    if (!email.trim()) {
      Alert.alert(t('auth.forgotPassword.errorTitle'), t('auth.forgotPassword.errorRequired'));
      return;
    }
    setLoading(true);
    ...
    setLoading(false);
    if (error) {
      Alert.alert(t('auth.forgotPassword.errorTitle'), t('auth.login.errorGeneric'));
    } else {
      Alert.alert(
