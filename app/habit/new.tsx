...
export default function NewHabitScreen() {
  ...
  const [notifType, setNotifType] = useState<NotificationType>('fixed');
  const [intervalMinutes, setIntervalMinutes] = useState('120');
  const [fixedTimes, setFixedTimes] = useState<string[]>(['09:00', '21:00']);
  const [smartStart, setSmartStart] = useState('09:00');
  const [smartEnd, setSmartEnd] = useState('21:00');
  ...
  const handleSave = async () => {
    ...
    else Alert.alert(t('common.error'), t('habitForm.errors.saveFailed'));
  };
    ...
    >
      ...
      >
        ...
        <TouchableOpacity
          ...
          disabled={saving}
        >
          <Text style={styles.saveBtnText}>{saving ? t('habitForm.saving') : t('habitForm.saveNew')}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // Target row
  targetRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 0 },
  targetSep: { width: 12 },
